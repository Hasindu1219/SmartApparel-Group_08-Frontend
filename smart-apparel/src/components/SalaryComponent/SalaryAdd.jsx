...
const SalaryAdd = () => {
    ...
    const handleSubmit = async () => {
        ...
        const isValid = validateEmpId() && validateStatus() && validateYearNMonth() && validateBasic() &&
            validateEpfByEmployee() && validateEpfByCompany() && validateEtfPayment() && validateNetSalary();

        if (isValid) {
            // Create the newSalary object only if all validations pass
            const newSalary = {
                empId: empId,
                status: status,
            ...
            try {
                ...
                if (response.status === 202) {
                    ...
                    alert("Salary already exists or invalid request.");
                } else {
                    // Other failure cases
                    alert("Error occurred while saving Salary Record.");
                }
            } catch (error) {
                console.error("Error occurred while adding salary:", error);
                alert("Error occurred while adding salary.");
            }
        } else {
        ...
        <>
            <Grid container spacing={2} sx={{ backgroundColor: '#EEEEEE', margin: 'auto', display: 'block' }}>
                ...
                <Grid item sx={{ display: 'flex' }}>
                    ...
                    <Input type="number" name="netSal" id="netSal" sx={{ width: '400px' }} value={netSalary} onChange={e => { setNetSalary(e.target.value) }} onBlur={validateNetSalary} />
