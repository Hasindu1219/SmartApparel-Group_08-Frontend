...
export default function ViewMaterials() {
  ...
  // Handling delete with confirmation
  const handleDeleteBtn = async (itemID) => {
    const confirmDelete = window.confirm("Are you sure you want to delete this item?");
    if (confirmDelete) {
      await axios
        .delete(`http://localhost:8080/smart-apperal/api/inventories/deleteItem/${itemID}`)
        .then((res) => {
          setDeleteItem(true);
  ...
  const handleEditBtn = async (itemID) => {
    await axios
      .get(`http://localhost:8080/smart-apperal/api/inventories/item/${itemID}`)
      .then((res) => {
        ...
        setPurchaseDate(new Date(res.data.purchaseDate)); // Assuming res.data.purchaseDate is in ISO format or Date object
  ...
  const handleUpdateBtn = async () => {
    if (!itemCategory || !itemName || !stockQty || !unit || !purchasePrice || !purchaseDate) {
      ...
      const updateData = {
        ...
        unit,
        purchasePrice,
        purchaseDate, // Assuming purchaseDate is a Date object
      };
      await axios
        .put(`http://localhost:8080/smart-apperal/api/inventories/updateItem`, updateData)
        .then((res) => {
        ...
        <div style={{ width: "100%", backgroundColor: "#d7e3fc", height: "100vh" }}>
          ...
          <div className="updateConatiner" style={{ display: modelView }}>
            <form action="">
              ...
              <div className="formBox">
                <label htmlFor="" style={{ marginRight: "6.8rem" }}>
                  Unit:{" "}
                </label>
                <select
                  value={unit}
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      <div className="tableBtn">
                        <button
                          className="editBtn"
                          onClick={() => handleEditBtn(data.itemID)}
                        >
                          Edit
                        </button>
                        <button
                          className="deleteBtn"
                          onClick={() => handleDeleteBtn(data.itemID)}
                        >
                          Delete
                        </button>
                      </div>
